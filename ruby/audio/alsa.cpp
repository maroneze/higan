...
public:
  ...
  void sample(uint16_t left, uint16_t right) {
    ...
    if(device.changed_frequency) {
      // prevents buffer issues due to size difference after changing frequency
      device.changed_frequency = false;
      buffer.length = 0;
      return;
    }
